...
class Menu:
    ...
    def __init__(self, center: Vec2D, buttons: list, is_vertical: bool):
        ...
        if is_vertical:
            ...
            self.start_pos = (center[0] - len(buttons) / 2 * (AppConfig.BUTTON_WIDTH +
            ...
            if len(buttons) % 2 == 0:
                ...
                self.start_pos = (self.start_pos[0] - AppConfig.BUTTON_WIDTH / 2, self.start_pos[1])
    ...
    def draw_horizontal(self):
        for i in range(len(self.buttons)):
            self.buttons[i].start_pos = (self.start_pos[0] + i * (AppConfig.BUTTON_WIDTH + AppConfig.BUTTON_MARGIN) - AppConfig.BUTTON_MARGIN, self.start_pos[1])
            self.buttons[i].draw()
